...
cpbkp = _role addaction ["Recruit Peacekeeper $200000","noscript.sqf",'[player] call interact_recruit_ai;',1,false,true,"","!curreccop and player distance copbackup <= 10 and count (units group player) < 5 and isNato"];
...
//================================== COP DELETE EVIDENCE ================================================
//action23 = _role addaction ["Take evidence","noscript.sqf",'player groupchat "evidence removed."; {deletevehicle _x} foreach (nearestobjects [getpos player, ["weaponholder"], 3])',1,true,true,"",'_weps = (nearestobjects [getpos player, ["weaponholder"], 3] select 0); player distance _weps < 5 and isGov'];
//================================== COP SLAVE ACTIONS ================================================
...
//=========================    Cop Patrol actions   ==============================
// !TODO Change to TNP
action87 = _role addaction["Get patrol mission","coppatrol.sqf",["start"],1,false,true,"","isNgov and (player distance copbank <= 4  or player distance atmtnpnc <= 4 or player distance atmtnpsc <= 4) and !pmissionactive and !patrolwaittime"];
action88 = _role addaction["Cancel patrol mission","coppatrol.sqf",["end"],1,false,true,"","isNgov and pmissionactive and (player distance copbank <= 4  or player distance atmtnpnc <= 4 or player distance atmtnpsc <= 4)"];
//========================   HOSTAGE  ================================================
...
//=======================   Road Gates ==============================================
/*
action92 = _role addaction ["Raise Gates","rgate1.sqf",[],1,false,true,"","isNato and player distance copgate1 <= 5"];
action93 = _role addaction ["Lower Gates","lgate1.sqf",[],1,false,true,"","isNato and player distance copgate1 <= 5"];
action94 = _role addaction ["Raise Gates","rgate2.sqf",[],1,false,true,"","isNato and player distance copgate2 <= 5"];
action95 = _role addaction ["Lower Gates","lgate2.sqf",[],1,false,true,"","isNato and player distance copgate2 <= 5"];
action96 = _role addaction ["Raise Gates","rgate3.sqf",[],1,false,true,"","isNato and player distance copgate3 <= 5"];
action97 = _role addaction ["Lower Gates","lgate3.sqf",[],1,false,true,"","isNato and player distance copgate3 <= 5"];
action98 = _role addaction ["Raise Gates","rgate4.sqf",[],1,false,true,"","isNato and player distance copgate4 <= 5"];
action99 = _role addaction ["Lower Gates","lgate4.sqf",[],1,false,true,"","isNato and player distance copgate4 <= 5"];
...
action102 = _role addaction ["Raise Gates","rgate5.sqf",[],1,false,true,"","isterror and player distance opforgate2 <= 5"];
action103 = _role addaction ["Lower Gates","lgate8.sqf",[],1,false,true,"","player distance opforgate2 <= 5"];
...
opforZone2Action = _role addaction ["Cap/Neutralize Khushab City", "noscript.sqf", "['opforZone2',opforZone2Flag,'opfor2Flag'] call zone_capture;",1,false,true,"","!isciv && player distance opforZone2Flag < 5 && side player != ['opforZone2'] call zone_getOwner"];
